...
function CrowFootEnd({
  ...
  return (
    <g transform={`translate(${x}, ${y})`}>
      <circle cx="0" cy="0" r="3" className="fill-edge" />
    </g>
  )
}
      ...
      <EdgeLabelRenderer>
        ...
        >
          <div className="flex items-center gap-1">
            <span className="text-[10px] font-mono text-muted-foreground bg-popover/90 px-1.5 py-0.5 rounded border border-border shadow-sm">
              {labelMap[relationType]}
            </span>
            {selected && (
              <button
                onClick={() => removeEdge(id)}
                className="w-5 h-5 rounded-full bg-destructive text-white flex items-center justify-center hover:bg-destructive/90 transition-colors shadow-sm"
              >
                <X className="w-3 h-3" />
              </button>
            )}
